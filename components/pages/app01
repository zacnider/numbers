...
// Ana uygulama bileşeni - blockchain kaydetme fonksiyonu eklendi
...
export default function App() {
  ...
  const {
    ...
    metamaskAddress,
    saveWalletsToBlockchain, // Yeni: Blockchain'e kaydet fonksiyonu
    isBlockchainSynced      // Yeni: Senkronizasyon durumu
  } = useWalletManager();
  ...
  const [isSavingToBlockchain, setIsSavingToBlockchain] = useState(false); // Yeni: Kaydetme durumu
  ...
  const handleSaveWallets = async (): Promise<boolean> => {
    if (isSavingToBlockchain) return false;
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 space-y-4">
            <WalletManager 
              ...
              onSaveWallets={handleSaveWallets} // Yeni: Blockchain'e kaydetme fonksiyonu
              isConnectedToMetaMask={isConnectedToMetaMask}
              metamaskAddress={metamaskAddress}
              isBlockchainSynced={isBlockchainSynced} // Yeni: Senkronizasyon durumu
              isSavingToBlockchain={isSavingToBlockchain} // Yeni: Kaydetme durumu
            />
