...
// Düzeltilmiş versiyon - blockchain işlemleri için özel anahtar kullanımı eklendi

import { useState, useEffect, useCallback } from 'react';
import { GAME_TIME_LIMIT } from '@/lib/constants';
import { useContract } from './useContract';
import { Wallet } from '@/types'; // Wallet tipini ekledik
...
// Tip uyumsuzluğu düzeltildi: currentWallet tipini Wallet | null olarak güncellendi
export function useGameState(size = 4, currentWallet: Wallet | null = null, walletBalance = '0') {
  ...
  const handleGameCompletion = useCallback(() => {
    ...
    (async () => {
      try {
        // Özel anahtarı doğrudan kullanarak blockchain'de işlem başlat
        if (currentWallet) {
          await completeGame(currentWallet.privateKey);
          console.log('Oyun tamamlama zincire kaydedildi');
        } else {
          await completeGame();
          console.log('Oyun tamamlama zincire kaydedildi (varsayılan yöntemle)');
        }
      } catch (error) {
  ...
  }, [timerId, completeGame, currentWallet]);
  ...
  const startGame = useCallback(() => {
    ...
    (async () => {
      try {
        // Özel anahtarı doğrudan kullanarak blockchain'de işlem başlat
        await startGameOnChain(currentWallet.privateKey);
  ...
  const handleTileClick = useCallback((row: number, col: number) => {
    ...
    (async () => {
      try {
        // Eğer mevcut cüzdan varsa, özel anahtarını kullanarak işlemi gönder
        if (currentWallet) {
          await makeMove(currentWallet.privateKey);
          console.log('Hamle zincire kaydedildi (cüzdan kullanarak)');
        } else {
          // Cüzdan yoksa standart makeMove'u dene
          await makeMove();
          console.log('Hamle zincire kaydedildi (varsayılan yöntemle)');
        }
      } catch (error) {
  ...
  }, [board, emptyTile, isGameActive, size, makeMove, handleGameCompletion, currentWallet]);
