...
export const useWalletManager = (): WalletManagerReturn => {
  ...
  const depositFunds = useCallback(async (amount: string): Promise<string> => {
    ...
    try {
      ...
      try {
        // Cüzdan adresini `0x${string}` formatına dönüştür
        // currentWallet.address zaten 0x ile başladığından emin oluyoruz
        const formattedAddress = currentWallet.address.startsWith('0x') 
          ? currentWallet.address as `0x${string}`
          : `0x${currentWallet.address}` as `0x${string}`;
